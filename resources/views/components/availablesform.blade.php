        ...
        <form class="space-y-4 md:space-y-6 p-5" action="{{ route('login') }}" method="post">
            ...
            <div>
                <label for="description"
                    class="block mb-2 text-sm font-medium text-[#181818] dark:text-white">Beschreibung</label>
                <textarea type="text" name="description" id="description" description
...
@endif
            <div class="flex items-center justify-between">
                <a href="{{ route('forgotpassword') }}" class="text-sm font-medium text-[#181818] hover:underline dark:text-primary-500">Passwort vergessen?</a>
            </div>

            <button type="submit" class="w-full text-white bg-[#376791] hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Anmelden</button>
            <p class="text-sm font-light text-[#181818] dark:text-gray-400">
                Noch kein Konto? <a href="{{ route('register') }}" class="font-medium text-[#181818] hover:underline dark:text-primary-500">Registrieren</a>
            </p>
        </form>
    </div>
</section>
